...
func _ready() -> void:
	visible = true
	#elevation = $ElevationsLayer ## keep this here for a moment, maybe its legacy code, maybe it served an important purpose
	player = get_tree().get_first_node_in_group('player')
	player.get_node(^'SnowTracker').reached_new_elevation.connect(_on_new_elevation)


func _on_new_elevation(tile: int = 0) -> void:
...
func _init_current_fogs(_tile: int) -> void:
...
func set_current_fogs(_tile: int) -> void:
