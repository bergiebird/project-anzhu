...
func _ready()->void:
	Signalton.new_hour_nightlight.connect(update_nightlight)
	parent.publish_event.connect(func(func_name, data :Variant=null):L.Observe.subscribe_to_event(self, func_name, data))
