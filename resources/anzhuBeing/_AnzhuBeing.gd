...
var current_direction :int = PersonalDirection.EAST:
	set(value): if current_direction != value:
		...
		direction_changed_named.emit(Directon.get_current_direction(current_direction))
...
func _process(delta:float)->void:
	...
	for being in being_process_array:
		being.being_process(delta)

func _physics_process(delta: float) -> void:
	__physics_process(delta)
	for being in being_physics_process_array:
...
func velocity_force_filter_for_direction(delta:float)->void:
	...
	if velocity_force != Vector2.ZERO:
		var new_direction = Directon.get_prevalent_direction(velocity_force)
...
func parse_incoming_damage(damage,weapon,who:AnzhuBeing)->bool: return true
