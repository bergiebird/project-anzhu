...
@onready var chase_timer :Timer = $Timer_Chase
...
func physics_update(delta :float)->void:
	...
	if grandparent.velocity.x > max_speed:
		grandparent.velocity.x = max_speed
	if grandparent.velocity.y > max_speed:
		grandparent.velocity.y = max_speed
	if grandparent.velocity.x < -max_speed:
		grandparent.velocity.x = -max_speed
	if grandparent.velocity.y < -max_speed:
		grandparent.velocity.y = -max_speed
