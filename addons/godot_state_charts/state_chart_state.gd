@tool class_name StateChartState extends Node
...
var engineton :Object = Engine

func _ready()->void:
	if engineton.is_editor_hint(): return
	_chart = _find_chart(get_parent())

func _find_chart(parent :Node)->StateChart:
	if parent is StateChart: return parent
...
func _run_transition(transition :Transition, immediately :bool=false)->void:
	...
	if not immediately and initial_delay > 0: _queue_transition(transition, initial_delay)
	else:                                     _chart._run_transition(transition, self)

func _state_init()->void: # disable state by default
	process_mode = Node.PROCESS_MODE_DISABLED
	_state_active = false
	_toggle_processing(false)
	_transitions.clear()
	for child in get_children():
		if child is Transition: _transitions.append(child)
...
## Called when the state should be saved. The parameter is is the SavedState object  of the parent state. The state is
## expected to add a child to the SavedState object under its own name.
...
func _state_save(saved_state :SavedState, child_levels :int=-1)->void:
	if not active:
		...
		return
	var our_saved_state :SavedState = SavedState.new()
	...
	if child_levels == 0: return
	for child in get_children():
		if child is StateChartState and child.active: child._state_save(our_saved_state, -1 if child_levels == -1 else child_levels - 1)

## Called when the state should be restored. The parameter is the SavedState object  of the parent state. The state is
## expected to retrieve the SavedState object for itself from the parent and restore its state from it.
...
func _state_restore(saved_state :SavedState, child_levels :int=-1)->void:
	...
	for child in get_children():
		if child is StateChartState: child._state_restore(our_saved_state, -1 if child_levels == -1 else child_levels - 1)
...
func _process_transitions(trigger_type:StateChart.TriggerType, event:StringName = "") -> bool: ## Processes all transitions in this state.
	if not active: return false
	if trigger_type == StateChart.TriggerType.EVENT: # emit an event received signal if this is an event trigger
		event_received.emit(event)
	for transition in _transitions: # Walk over all transitions
		if transition.is_triggered_by(trigger_type) and (event == "" or transition.event == event) and transition.evaluate_guard():
				#print(name +  ": consuming event " + event)
				# first match wins
				# if the winning transition is the currently pending transition, we do not replace it
				if transition != _pending_transition: _run_transition(transition)
				# but in any case we return true, because we consumed the event
				return true
	return false

func _queue_transition(transition:Transition, initial_delay:float): ## Queues the transition to be triggered after the delay.
	#print("transitioning from " + name + " to " + transition.to.get_concatenated_names() + " in " + str(transition.delay_seconds) + " seconds" )
	_pending_transition = transition # queue the transition for the delay time (0 means next frame)
...
func _get_configuration_warnings()->PackedStringArray:
	...
	if not found: result.append("State is not a child of a StateChart. This will not work.")
	return result

func _toggle_processing(active :bool)->void:
...
func _state_step()->void:
	state_stepped.emit() ## Called when the state chart step function is called.
