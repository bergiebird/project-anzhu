...
func perform(): #add Scene selector
...
func init_scene_selector(tab: int = -1):
	...
	for index in scenes.size():
		var scene_file = scenes.keys()[index]
		...
		scene_selector.get_popup().set_item_as_radio_checkable(index, false)
		scene_selector.set_item_metadata(index, scene_file)
		scene_selector.set_item_tooltip(index, scene_file)
